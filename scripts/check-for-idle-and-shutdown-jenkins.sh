...
#for url in $(docker_run curl -s http://$JENKINS_API_USERNAME:$JENKINS_API_TOKEN@build.ngip.io/jenkins/api/json | docker_run jq -r ".jobs[]? | .url"); do
  ...
  for job_url in $(docker_run curl -s http://$JENKINS_API_USERNAME:$JENKINS_API_TOKEN@build.ngip.io/jenkins/job/ngip/api/json | docker_run jq -r '.jobs[]? | .url'); do
    echo ${job_url}
    last_build_url=$(docker_run curl -s "${job_url/build.ngip.io/$JENKINS_API_USERNAME:$JENKINS_API_TOKEN@build.ngip.io}api/json" | docker_run jq -r ".lastBuild? | .url")
    if [ -n "$last_build_url" ]; then
      timestamp=$[($(date +%s)*1000 - $(curl -s "${last_build_url/build.ngip.io/$JENKINS_API_USERNAME:$JENKINS_API_TOKEN@build.ngip.io}api/json" | docker_run jq -r ".timestamp"))/1000]
